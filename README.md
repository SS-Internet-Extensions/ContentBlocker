...
- Context options: common request type and context options such as `$script`, `$image`, `$stylesheet`, `$css`, `$subdocument`, `$frame`, `$xmlhttprequest`, `$xhr`, `$third-party`, `$3p`, `$1p`, `$domain=...`, `$from=...`, `$to=...`, `$denyallow=...`, `$method=...`, `$match-case`, `$genericblock`, `$popup`, `$redirect=...`, and `$removeparam=...`. Custom and engine-specific network options are preserved so custom filters are not disabled by the compatibility layer.
...
- `$important`, `$badfilter`, exception rules, request type matching, `$method=...`, `$denyallow=...`, `$from=...`, `$to=...`, `$genericblock`, case-insensitive URL matching by default, `$match-case`, `$third-party`, `$3p`, `$1p`, `$~third-party`, `$~3p`, `$~1p`, and `$domain=...` context.
