...
- `$important`, `$badfilter`, exception rules, request type matching, `$method=...`, `$denyallow=...`, case-insensitive URL matching by default, `$match-case`, `$third-party`, `$3p`, `$1p`, `$~third-party`, `$~3p`, `$~1p`, and `$domain=...` context.
